...
public class ProductBatchConfig {
	...
	LineMapper<ProductEntity> lineMapper(){
		...
		lineTokenizer.setNames("id","category","description","image_url","name","price");
	...
	public RepositoryItemWriter<ProductEntity> writer(){
		...
		writer.setRepository(productRepo);
		writer.setMethodName("save");
		return writer;
	}
	
	@Bean
	public Step step(ItemReader<ProductEntity> reader, ItemProcessor<ProductEntity,ProductEntity> processor, ItemWriter<ProductEntity> writer, JobRepository jobRepository, PlatformTransactionManager manager) {
		return new StepBuilder("My Step", jobRepository).<ProductEntity,ProductEntity>chunk(10,manager).reader(reader).processor(processor).writer(writer).build();
	}
	
	@Bean
	public Job runJob(Step step, JobRepository jobRepository) {
		return new JobBuilder("My Job",jobRepository).incrementer(new RunIdIncrementer()).flow(step).end().build();
	}
